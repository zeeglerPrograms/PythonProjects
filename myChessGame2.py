# to add : 
    # pawn can en pessant
        # how to get moveorder into pawnmoves?
    ...
    # check mate
        # right now the user has to input checkmate. Lets automate that
...
class Piece():
    ...
    def move_piece(self, squares_taken):
        ...
        if self.name[1] == 'b' or self.name[1] == 'q':
            ...
            self.attacking_squares = self.moves
...
class Board:
    ...
    def find_squares_attacked(self, player):
        squares_attacked = []
        if player.color == 'w':
            for piece in self.black_pieces:
                squares_attacked.append(piece.attacking_squares)
                #testing
                #squares_attacked.append(f'{piece.name}: {piece.attacking_squares}')
...
class Game():
    ...
    def game_loop(self):
        ...
        while check_mate == False and running == True:
            ...
            """ # checking out find squares attacked
            print()
            other_player_is_attacking = board.find_squares_attacked(player_turn_ref)
            for piece in other_player_is_attacking:
                if 'a6' in piece:
                    print('a6 is in')"""
            ...
            while piece_moved == False:
                if player_turn_ref.checked == True:
                    ...
                    print('If you are checkmated, type \'cm\' and the game will end.')
                ...
                if this_piece_location in squares_taken:
                        ...
                        if 'x' in move_piece_to:
                            ...
                            if player_turn_ref.checked == True:
                                ...
                                if player_turn_ref.checked == True:
                                    ...
                                    break
                            self.move_order.append(f'{piece.name}: {piece.pos}')
                            piece_moved_complete = True
                            piece_moved = True
                            ...
                            if piece.value == 1:
                                ...
                                name_to_show = piece.display_name[1]
                            self.move_order.append(f'{name_to_show}{piece_old_pos}: {piece.pos}')
                            piece_moved = True
                            piece_moved_complete = True
                    ...
                    if this_piece_location == 'cm':
                        if player_turn_ref.color == 'w':
                            player_checkmated = 'White'
                        if player_turn_ref.color == 'b':
                            player_checkmated = 'Black'
                        print(f'The game is over. {player_checkmated} was Checkmated.')
                        check_mate = True
                        piece_moved = True
                        piece_moved_complete = True
                        running = False
            ...
            if this_piece_location == 'q':
                print('Thanks for playing')
                print('Here is the move order from the game:')
